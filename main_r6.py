...
def draw_player_turn_button(screen, button_text, message=""):
    ...
    # 10 pixels margin from the right edge
...
def draw_hex_shape_grid(surface, center_row, center_col, size):
    ...
    def get_hex_label(row, col, max_dist):
        ...
        corners = [(row, col) for row in (-max_dist, max_dist)
                   for col in (-max_dist, max_dist)]
        corners.extend([(-max_dist, 0), (0, -max_dist),
...
def update_selected_hexes(selected_hexes):
    ...
    for hex_info in selected_hexes:
        ...
        # Update the owner when a hex is selected
        hex_info['owner'] = current_turn
        selected_counts[hex_info['label']] = selected_counts.get(
            hex_info['label'], 0) + 1
        fill_color = BLACK if current_turn == 'black' else WHITE
        draw_hexagon(screen, hex_info['x'], hex_info['y'], HEX_SIZE,
                     (128, 128, 128), fill_color, hex_info['label'])
        pygame.display.flip()
    ...
    for (hx, hy), hex_info in hexagon_board.items():
        if point_in_hex(x, y, hex_info['x'], hex_info['y'], HEX_SIZE) and not hex_info.get('booked', False):
            if current_round == 1 and hex_info['label'] == 2 and selected_counts.get(2, 0) < 1:
                ...
                # In the first round, only one hexagon labeled as 2 is allowed to be selected.
                break
                ...
                if selected_counts.get(current_label, 0) >= required_selections:
                    # Stop selecting once the required number is reached.
                    break
...
def auto_select_remaining_hexes(label, required_selections):
    ...
    number_to_select = min(len(remaining_hexes),
                           required_selections - selected_counts.get(label, 0))

    selected_hexes = random.sample(
        remaining_hexes, number_to_select) if number_to_select > 0 else []
    # Process selected hexes
    update_selected_hexes(selected_hexes)

    print(
        f"AI selected {number_to_select} hexes automatically due to timeout.")
...
def end_current_round():
    ...
    if current_label is None:
        if current_round != 1:
            print(
                "No selections have been made this round. You must select at least one hex.")
    ...
    remaining_hexes = len([info for info in hexagon_board.values(
    ) if info['label'] == current_label and not info.get('booked', False)])
    selected_hexes_count = selected_counts.get(current_label, 0)

    if (remaining_hexes == 0) and (selected_hexes_count + 1) == current_label:

        print(
            f"Ending round with sufficient selections for label: {current_label}.")
    ...
    # Check if the round can be ended
        ...
        if selected_hexes_count != current_label:
            ...
            print(
                f"You have not selected enough hexes of label: {current_label}. Needed: {current_label}, selected: {selected_hexes_count}.")
            return

        elif remaining_hexes < current_label:
            if selected_hexes_count < remaining_hexes:
                print(
                    f"Not enough hexes left for label: {current_label}. You need to select all {remaining_hexes} available hexes.")
        ...
        for label in list(hexes_by_label.keys()):
            ...
            if label not in combinations_cache:
                if len(hex_list) < label_int:
                    ...
                    combinations_cache[label] = list(itertools.combinations(hex_list, label_int))

            for combination in combinations_cache[label]:
                futures.append(executor.submit(process_combination, label_int, combination))
...
def main(black_player, white_player):
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                if (current_turn == 'black' and black_player_type == "human") or \
                   (current_turn == 'white' and white_player_type == "human"):
                    ...
                    # 手動選擇座標
                    selected_hexes = process_selections_by_round(
                        x, y, current_round)
                    # 更新座標
                    update_selected_hexes(selected_hexes)
            ...
            if not turn_ended and ((current_turn == 'black' and black_player_type == "ai") or
                ...
                selected_hexes = select_hexes_by_ai(
                    hexes_by_label, current_round, current_turn)
            ...
            if not turn_ended and ((current_turn == 'black' and black_player_type == "random") or
                ...
                selected_hexes = select_hexes_by_random(
                    hexes_by_label, current_round)
